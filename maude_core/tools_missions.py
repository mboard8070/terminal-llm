...
_DEFAULT_MISSIONS_DIR = _ROOT / "data" / "missions"
...
def _missions_dir() -> Path:
    path = Path(os.environ.get("MAUDE_MISSIONS_DIR", _DEFAULT_MISSIONS_DIR))
...
def _dispatch_mission_run_next(args: dict[str, Any]) -> str:
    ...
    from .tools_plan import execute_plan
...
def _dispatch_mission_tick(args: dict[str, Any]) -> str:
    ...
    step = next((item for item in mission.get("steps", []) if item.get("status") in {"pending", "in_progress", "blocked"}), None)
    if not step:
...
def _dispatch_mission_drain(args: dict[str, Any]) -> str:
    ...
    header = f"Mission drain ran {len([item for item in results if item.startswith('Mission step ')])} executable step(s)."
...
def _dispatch_mission_brief(args: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Next action:",
        next_step["title"] if next_step else "No pending steps. Review success criteria and close or extend the mission.",
    ]
