...
_FAST_PATTERNS = [
    ...
    (
        ...
        "drive_search",
        lambda m, msg: {"query": _re.sub(r".*?(?:search|find|look for)\s+", "", msg, flags=_re.I).strip().rstrip("?.")},
    ),
    (
        _re.compile(
    ...
    (
        _re.compile(
            r"\b(?:search|google|look up|what is|what are|who is|when is|where is)\b(?!.*\b(?:doc|drive|sheet|calendar|slide|contact|gmail|emails?|mail|inbox)\b)",
            _re.I,
        ),
        "web_search",
        lambda m, msg: {
