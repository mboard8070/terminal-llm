...
class ProactiveScheduler:
    ...
    CONFIG_FILE = runtime_paths().schedules_file
    ...
    def _mission_report_from_state(self, task: ScheduledTask, result: str) -> str | None:
        ...
        missions_dir = runtime_paths().missions_dir
        ...
        latest = None
        for step in steps:
            if step.get("last_run_at") and (
                latest is None or str(step.get("last_run_at")) > str(latest.get("last_run_at"))
            ):
                latest = step
        if latest is None:
