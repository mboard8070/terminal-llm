...
class RoutingDecision:
    ...
    subagent: str | None = None  # Which subagent to route to (None = main LLM)
    prefer_cloud: bool = False  # Whether to prefer cloud for this request
    prefer_model: str | None = None  # Short model name to auto-switch to (e.g. "codestral")
    context_hints: dict | None = field(  # Extra context to inject
...
class _WeightedPattern:
    ...
    pattern: re.Pattern
    weight: float
    hint_key: str | None = None  # Optional key to set in context_hints
    hint_value: str | None = None  # Value for the hint
...
_INTENT_TO_SUBAGENT: dict[str, str | None] = {
...
_INTENT_TO_MODEL: dict[str, str | None] = {
...
def _compile(pattern: str, weight: float, hint_key: str = None, hint_value: str = None) -> _WeightedPattern:
...
def _build_all_patterns() -> dict[str, list[_WeightedPattern]]:
    ...
    patterns["code"] = [
        ...
        _compile(
            r"\b(write|create|generate|build|implement)\b.{0,20}\b(function|class|method|module|script|program|api|endpoint|handler|middleware|decorator|component)\b",
            0.9,
        ),
        _compile(
            r"\b(fix|debug|solve|troubleshoot|diagnose)\b.{0,20}\b(bug|error|issue|crash|exception|traceback|segfault|segmentation)\b",
            0.9,
        ),
        ...
        _compile(
            r"\b(python|javascript|typescript|java|c\+\+|rust|go(lang)?|ruby|swift|kotlin|scala|haskell|elixir|clojure|dart|php|perl|lua|zig|nim)\b",
            0.5,
            "language",
        ),
        _compile(
            r"\b(react|vue|angular|svelte|nextjs|next\.js|django|flask|fastapi|express|spring|rails|laravel)\b",
            0.55,
            "framework",
        ),
        ...
        _compile(
            r"\b(optimize|refactor|clean\s*up|simplify|improve)\b.{0,15}\b(code|function|class|algorithm|logic|performance)\b",
            0.85,
        ),
        _compile(
            r"\b(add|implement|create)\b.{0,15}\b(unit\s*test|test\s*case|integration\s*test|e2e\s*test|mock|fixture|test)\b",
            0.8,
        ),
        _compile(
            r"\b(how\s*(do|can|to|does))\b.{0,15}\b(import|install|require|include|use|call|invoke|initialize)\b", 0.5
        ),
        # Code patterns and concepts
        _compile(r"\bregex\b|\bregular\s*expression\b|\bregexp\b", 0.9),
        _compile(
            r"\b(algorithm|data\s*structure|linked\s*list|binary\s*tree|hash\s*map|hash\s*table|graph\s*algorithm|sort(ing)?|search(ing)?)\b",
            0.6,
        ),
        _compile(r"\b(async|await|promise|callback|thread|mutex|semaphore|coroutine|concurrency|parallelism)\b", 0.55),
        _compile(
            r"\b(docker(file)?|kubernetes|k8s|helm|terraform|ansible|ci\s*/?\s*cd|github\s*actions?|jenkins|pipeline)\b",
            0.5,
            "devops",
        ),
        ...
        _compile(
            r"\b(syntax\s*error|type\s*error|name\s*error|import\s*error|attribute\s*error|value\s*error|runtime\s*error|compilation\s*error|linker\s*error)\b",
            0.85,
        ),
        _compile(r"\b(stack\s*trace|traceback|exception|error\s*message|undefined|null\s*pointer|nil|NoneType)\b", 0.7),
        _compile(
            r"\b(variable|parameter|argument|return\s*type|type\s*hint|annotation|generic|interface|abstract|inheritance|polymorphism)\b",
            0.45,
        ),
        ...
        _compile(
            r"\b(pip|npm|yarn|cargo|gem|maven|gradle|composer|nuget|brew)\s+(install|add|update|upgrade|remove)\b", 0.7
        ),
    ...
    patterns["search"] = [
        ...
        _compile(
            r"\b(search|look\s*up|google|find\s*(out|me|information|info))\b.{0,20}\b(about|for|on|regarding)\b", 0.9
        ),
        _compile(
            r"\b(what('s| is| are)|who('s| is| are)|when (is|was|did|does|will)|where (is|are|was|did|can))\b.{0,30}\b(latest|current|recent|now|today|tonight|this\s*(week|month|year))\b",
            0.9,
        ),
        # Current events and news
        _compile(r"\b(news|headline|breaking|current\s*events?|what('s| is)\s*happening)\b", 0.85),
        _compile(
            r"\b(latest|recent|new|upcoming|current)\b.{0,20}\b(update|release|version|announcement|report)\b", 0.7
        ),
        _compile(
            r"\b(today|tonight|yesterday|this\s*week|this\s*month|right\s*now|currently)\b.{0,15}\b(weather|temperature|forecast|traffic|score|result|price|rate)\b",
            0.9,
        ),
        # Real-time data
        _compile(r"\b(stock|share)\s*(price|market|ticker|symbol|quote)\b", 0.9, "data_type", "stocks"),
        _compile(
            r"\b(bitcoin|ethereum|btc|eth|crypto(currency)?|coin)\s*(price|value|market|cap)\b",
            0.9,
            "data_type",
            "crypto",
        ),
        ...
        _compile(
            r"\b(exchange\s*rate|forex|usd|eur|gbp|jpy)\b.{0,15}\b(to|rate|convert|price)\b", 0.8, "data_type", "forex"
        ),
        _compile(
            r"\b(score|result|standing|ranking|fixture)\b.{0,15}\b(game|match|nfl|nba|mlb|nhl|premier\s*league|soccer|football|basketball|baseball|hockey)\b",
            0.9,
            "data_type",
            "sports",
        ),
        ...
        _compile(
            r"\b(who\s*(won|lost|beat|scored|plays?|playing))\b.{0,25}\b(game|match|nfl|nba|mlb|nhl|premier\s*league|series|cup|championship|tournament|super\s*bowl|world\s*series)\b",
            0.9,
            "data_type",
            "sports",
        ),
        _compile(
            r"\b(nfl|nba|mlb|nhl|premier\s*league|champions\s*league|world\s*cup|super\s*bowl|march\s*madness)\b.{0,20}\b(score|result|winner|game|match|schedule|standing|playoff)\b",
            0.85,
            "data_type",
            "sports",
        ),
        _compile(
            r"\b(last\s*night|yesterday|tonight|this\s*weekend)\b.{0,15}\b(game|match|fight|bout|race)\b",
            0.75,
            "data_type",
            "sports",
        ),
        ...
        _compile(
            r"\b(weather|forecast|temperature)\b.{0,15}\b(in|for|at|today|tomorrow|this\s*week)\b",
            0.85,
            "data_type",
            "weather",
        ),
        _compile(
            r"\b(flight|flights?)\b.{0,15}\b(status|delayed|from|to|price|cheap|book)\b", 0.7, "data_type", "travel"
        ),
        _compile(
            r"\b(restaurant|hotel|store|shop)\b.{0,15}\b(near|nearby|close|around|in|open|hours|review)\b",
            0.65,
            "data_type",
            "local",
        ),
        ...
        _compile(
            r"\b(schedule|event|concert|show|movie|film)\b.{0,15}\b(today|tonight|this\s*week|near|showing|playing|time)\b",
            0.7,
        ),
        _compile(
            r"\b(what\s*time|when)\b.{0,20}\b(game|match|show|movie|concert|event|flight)\b.{0,15}\b(start|begin|on|tonight|today|tomorrow)\b",
            0.85,
            "data_type",
            "events",
        ),
        _compile(
            r"\b(game|match)\b.{0,15}\b(start|begin|time|tonight|today|tomorrow|this\s*week)\b",
            0.6,
            "data_type",
            "sports",
        ),
    ...
    patterns["vision"] = [
        ...
        _compile(
            r"\b(look\s*at|analyze|describe|examine|inspect|read|scan|check)\b.{0,15}\b(this\s*)?(image|picture|photo|screenshot|screen\s*cap|snap|diagram|chart|graph|figure|illustration)\b",
            0.95,
        ),
        ...
        _compile(
            r"\b(ui|ux|interface|layout|design|mockup|wireframe)\b.{0,15}\b(feedback|review|analyze|issue|problem|bug)\b",
            0.7,
        ),
        _compile(r"\b(this\s*error|error\s*message)\b.{0,10}\b(screen|shows?|says?|displays?)\b", 0.6),
        _compile(
            r"\b(chart|graph|plot|diagram|table|infographic|dashboard)\b.{0,15}\b(shows?|means?|explain|interpret|read|understand)\b",
            0.8,
        ),
        ...
        _compile(
            r"\b(what\s*does)\b.{0,15}\b(this|the)\b.{0,10}\b(show|display|depict|illustrate|mean|say|represent)\b", 0.6
        ),
        # Attached/uploaded content
        _compile(
            r"\b(attached|uploaded|here('s| is)|see\s*(the|this))\b.{0,15}\b(image|picture|photo|screenshot|file)\b",
            0.85,
        ),
    ...
    patterns["writing"] = [
        ...
        _compile(
            r"\b(write|draft|compose|create|author)\b.{0,40}\b(email|e-mail|letter|memo|message|note|correspondence)\b",
            0.9,
            "writing_type",
            "email",
        ),
        _compile(
            r"\b(write|draft|compose|create)\b.{0,40}\b(blog\s*post|article|essay|paper|report|whitepaper|case\s*study)\b",
            0.9,
            "writing_type",
            "article",
        ),
        _compile(
            r"\b(write|draft|compose|create)\b.{0,40}\b(documentation|docs?|readme|guide|tutorial|manual|handbook|wiki)\b",
            0.85,
            "writing_type",
            "documentation",
        ),
        _compile(
            r"\b(write|draft|compose|create)\b.{0,40}\b(proposal|pitch|brief|plan|specification|spec|rfc)\b",
            0.85,
            "writing_type",
            "proposal",
        ),
        _compile(
            r"\b(write|draft|compose|create)\b.{0,40}\b(story|poem|narrative|fiction|screenplay|script|dialogue|monologue)\b",
            0.85,
            "writing_type",
            "creative",
        ),
        _compile(
            r"\b(write|draft|compose|create)\b.{0,40}\b(resume|cv|cover\s*letter|bio|biography|about\s*me|introduction)\b",
            0.85,
            "writing_type",
            "professional",
        ),
        ...
        _compile(
            r"\b(tldr|tl;dr|too\s*long|brief\s*overview|key\s*points?|main\s*points?|takeaways?|highlights?)\b",
            0.75,
            "writing_type",
            "summary",
        ),
        ...
        _compile(
            r"\b(rewrite|rephrase|paraphrase|improve|polish|revise|edit|proofread|copyedit)\b.{0,15}\b(this|the|my|following|text|paragraph|sentence|draft|document|article)\b",
            0.8,
        ),
        _compile(
            r"\b(make\s*(it|this))\b.{0,15}\b(more\s*)?(formal|informal|professional|casual|concise|clear|readable|engaging|persuasive|friendly|shorter|longer)\b",
            0.75,
        ),
        _compile(r"\b(tone|voice|style)\b.{0,10}\b(change|adjust|modify|more|less)\b", 0.7),
        # Content structure
        _compile(
            r"\b(outline|structure|organize|format|layout)\b.{0,15}\b(for|of|the|this|my|a)\b.{0,15}\b(document|article|paper|post|report|presentation)\b",
            0.75,
        ),
        _compile(r"\b(table\s*of\s*contents|section|chapter|heading|bullet\s*points?|numbered\s*list)\b", 0.4),
        # Professional/business writing
        _compile(
            r"\b(meeting\s*notes?|minutes|agenda|action\s*items?|follow[\s-]*up)\b", 0.65, "writing_type", "meeting"
        ),
        _compile(
            r"\b(announcement|press\s*release|newsletter|marketing\s*copy|ad\s*copy|tagline|slogan|headline)\b",
            0.75,
            "writing_type",
            "marketing",
        ),
        _compile(
            r"\b(business\s*plan|executive\s*summary|mission\s*statement|vision\s*statement)\b",
            0.8,
            "writing_type",
            "business",
        ),
        ...
        _compile(
            r"\b(translate|translation)\b.{0,40}\b(to|into|from)\b.{0,20}\b(english|spanish|french|german|chinese|japanese|korean|italian|portuguese|arabic|hindi|russian)\b",
            0.8,
            "writing_type",
            "translation",
        ),
        ...
        _compile(
            r"\b(detailed|comprehensive|thorough|in[\s-]*depth|extensive|complete)\b.{0,15}\b(explanation|overview|analysis|description|guide|breakdown)\b",
            0.6,
        ),
        _compile(
            r"\b(explain|describe|elaborate)\b.{0,10}\b(in\s*detail|thoroughly|step\s*by\s*step|at\s*length|comprehensively)\b",
            0.55,
        ),
    ]
    ...
    patterns["reasoning"] = [
        ...
        _compile(
            r"\b(analyze|analysis|evaluate|assess|critique|appraise)\b.{0,15}\b(the|this|my|these|those|following)\b",
            0.75,
        ),
        ...
        _compile(
            r"\b(advantages?\s*(and|&|\/)\s*disadvantages?|strengths?\s*(and|&|\/)\s*weaknesses?|benefits?\s*(and|&|\/)\s*drawbacks?)\b",
            0.85,
        ),
        _compile(r"\b(trade[\s-]*offs?|trade[\s-]*off)\b", 0.7),
        # Planning and strategy
        _compile(
            r"\b(plan|planning|strategy|strategize|roadmap|game\s*plan|action\s*plan)\b.{0,15}\b(for|to|how|the|my|a)\b",
            0.8,
        ),
        _compile(r"\b(step[\s-]*by[\s-]*step|step\s*by\s*step|walkthrough|walk\s*me\s*through)\b", 0.6),
        _compile(
            r"\b(how\s*(should|would|can|do)\s*(I|we|you))\b.{0,20}\b(approach|tackle|handle|solve|address|deal\s*with|manage|navigate|proceed|start)\b",
            0.8,
        ),
        _compile(
            r"\b(best\s*(approach|way|method|strategy|practice)|optimal|most\s*effective|most\s*efficient)\b", 0.6
        ),
        _compile(
            r"\b(decision|decide|choose|choosing|choice|pick|select|which\s*(one|option|approach))\b.{0,10}\b(between|should|is\s*better|makes?\s*sense|would\s*you)\b",
            0.85,
        ),
        ...
        # Complex analysis
        _compile(
            r"\b(root\s*cause|why\s*(does|did|is|are|was|were|do|would))\b.{0,20}\b(happen|fail|work|occur|exist|matter)\b",
            0.7,
        ),
        _compile(
            r"\b(implications?|consequences?|impact|ramifications?|repercussions?|side\s*effects?)\b.{0,10}\b(of|for|on)\b",
            0.7,
        ),
        _compile(r"\b(what\s*(if|would\s*happen)|hypothetic(al|ally)|scenario|thought\s*experiment)\b", 0.75),
        _compile(
            r"\b(risk|risks?)\b.{0,10}\b(analysis|assessment|mitigation|management|of|associated|involved)\b", 0.8
        ),
        ...
        _compile(
            r"\b(complex|complicated|intricate|nuanced|multi[\s-]*faceted|multi[\s-]*step|non[\s-]*trivial)\b.{0,15}\b(problem|question|issue|task|challenge|situation)\b",
            0.75,
        ),
        _compile(
            r"\b(break\s*(down|apart)|decompose|dissect|unpack)\b.{0,15}\b(this|the|problem|issue|question|concept)\b",
            0.7,
        ),
        _compile(r"\b(help\s*me\s*(think|understand|figure\s*out|work\s*through|reason))\b", 0.75),
        # Architecture and design decisions
        _compile(
            r"\b(architect(ure)?|system\s*design|design\s*pattern|design\s*decision)\b.{0,15}\b(for|of|the|should|best|recommend)\b",
            0.7,
        ),
        _compile(
            r"\b(when\s*(should|to)\s*(use|choose|pick|prefer))\b.{0,15}\b(over|instead\s*of|vs\.?|versus)\b", 0.75
        ),
        ...
        _compile(
            r"\b(probability|statistic(s|al)|correlation|regression|distribution|standard\s*deviation|mean|median|variance)\b",
            0.5,
        ),
    ]
    ...
    patterns["social_media"] = [
        _compile(
            r"\b(post|share|publish)\b.{0,15}\b(to|on)\b.{0,10}\b(twitter|x\.com|linkedin|facebook|instagram|mastodon|bluesky|threads|reddit|tiktok)\b",
            0.95,
        ),
        ...
        _compile(
            r"\b(hashtag|caption|viral|engagement|followers?|likes?|repost)\b.{0,15}\b(for|on|about|strategy)\b", 0.6
        ),
    ...
    patterns["browser"] = [
        _compile(
            r"\b(go\s*to|navigate\s*to|open|visit|browse)\b.{0,10}\b(the\s*)?(website|webpage|page|site|url|link)\b",
            0.9,
        ),
        _compile(r"\b(go\s*to|navigate\s*to|open|visit)\b.{0,5}(https?://|www\.)\S+", 0.95),
        _compile(
            r"\b(fill\s*(out|in)|complete|submit)\b.{0,30}\b(the\s*)?(form|field|input|application|survey)\b", 0.9
        ),
        _compile(
            r"\b(click|tap|press|select|choose)\b.{0,10}\b(the\s*)?(button|link|menu|dropdown|checkbox|radio|tab|option|element)\b",
            0.85,
        ),
        _compile(r"\b(scroll|scroll\s*down|scroll\s*to|swipe)\b", 0.6),
        _compile(
            r"\b(browser|chrome|firefox|selenium|playwright|puppeteer)\b.{0,10}\b(automation|automate|open|launch|use)\b",
            0.85,
        ),
        _compile(r"\b(scrape|crawl|extract)\b.{0,15}\b(from|the|this)\b.{0,10}\b(website|page|site|url)\b", 0.8),
        _compile(
            r"\b(log\s*in|login|sign\s*in|authenticate)\b.{0,10}\b(to|on|at)\b.{0,10}\b(the\s*)?(website|site|page|portal|account)\b",
            0.85,
        ),
...
class MessageRouter:
    ...
    def __init__(self) -> None:
        ...
        self._max_scores: dict[str, float] = {
            intent: sum(p.weight for p in pats) for intent, pats in self._patterns.items()
        }
    ...
    def route(self, message: str, history: list[dict] = None) -> RoutingDecision:
        ...
        if best_intent == "code" and context_hints and context_hints.get("ambiguous_with") == "reasoning":
    ...
    def get_intent_categories(self) -> list[str]:
        ...
        return [*self._patterns.keys(), "general"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        for msg in test_cases:
            ...
            print(
                f"  [{decision.intent:13}] {conf_bar:20} {decision.confidence:.2f}  -> {agent_label:10}{cloud_flag}{model_flag}  | {msg[:60]}"
            )
