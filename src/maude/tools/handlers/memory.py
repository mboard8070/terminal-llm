...
from maude.memory.ledger import LEDGER_CATEGORIES, MEMORY_TYPE_DESCRIPTIONS, get_ledger, normalize_memory_category
from maude_core.log import log
from maude_core.memory_utils import get_memory
from maude_core.mempalace_utils import get_palace_kg, get_palace_stack
...
def _dispatch_save_memory(args):
    ...
    category = normalize_memory_category(args.get("category", "semantic"))

    if not key or not value:
        return "Error: Both 'key' and 'value' are required."

    ledger = get_ledger()
    record = ledger.save(key, value, category)
...
def _dispatch_recall_memory(args):
    ...
    category = normalize_memory_category(args.get("category")) if args.get("category") else None
...
def _dispatch_memory_ledger_status(args):
    ...
    lines = [
        ...
        f"Verification gate: {status.get('verification_gate', 'unknown')}",
    ...
    for category in LEDGER_CATEGORIES:
        if category in MEMORY_TYPE_DESCRIPTIONS and category not in ("fact", "task", "conversation"):
            lines.append(f"- {category}: {MEMORY_TYPE_DESCRIPTIONS[category]}")
    if status["files"]:
