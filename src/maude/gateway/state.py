...
# Resolve runtime paths from environment-aware MAUDE configuration.
...
MODEL_ROUTES = {
    ...
    # Nemotron 3 Ultra — cloud via OpenRouter (free, 1M context)
    ...
    # xAI Grok via X Premium OAuth (no separate API key)
    "grok-4.3": {
        "provider": "xai",
        "base_url": "https://api.x.ai",
        "api_key_env": "",  # special-cased via xai_oauth for Premium accounts
        "max_context": 128000,
    },
}
...
TOOL_ADDENDUM = (
    ...
    f"and saved on this server in {SHARED_DIR}. "
    ...
    "\n\nVIDEO CONTENT GUARDRAILS: When creating, editing, rendering, or publishing video, "
    "do not rush source footage. Screen wipes, reveals, before/after clips, app recordings, "
    "and product demos need enough duration to complete and enough hold time to be understood. "
    "Never burn raw URLs or long links into the video frame unless explicitly requested; use a "
    "designed CTA on screen and put the real clickable URL in the post/video description or caption. "
    "Preview the rendered MP4 before publishing. Before youtube_upload or social_post with video, "
    "call video_pre_publish_checklist and create the checklist artifact; do not publish if it is BLOCKED. "
    "Do not publish if media is missing, text is unreadable, a transition is clipped, or a placeholder/raw link is visible."
    ...
    "\n\nPERSISTENT MEMORY: save_memory and recall_memory are MAUDE's front-door "
    "memory ledger tools. Use save_memory PROACTIVELY when the user shares "
    "personal facts, preferences, names, dates, project details, mission notes, "
    "or artifacts worth remembering across sessions. Do NOT ask permission to "
    "save — just save it. Use recall_memory when the user references something "
    "from a previous conversation or when context from past sessions would help. "
    "Categories: fact, preference, person, task, project, mission, artifact. "
    "Use MemPalace only as an optional deep archive when recall_memory is not enough."
)
