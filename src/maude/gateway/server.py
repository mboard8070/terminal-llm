...
"""
    ...
    def _route_model_request(self, pre_parsed_req=None):
        ...
        if route["provider"] == "xai":
            try:
                from maude.integrations.xai_oauth import get_oauth_access_token

                api_key = get_oauth_access_token()
            except Exception as exc:
                self._json_response({"error": f"xAI OAuth unavailable: {exc}"}, 503)
                return
        else:
