...
def _latest_shared_video(max_age_hours: int = 48) -> str | None:
    shared_dirs = [runtime_paths().shared_dir]
    legacy_shared = Path(__file__).resolve().parent / "shared"
    if legacy_shared not in shared_dirs:
        shared_dirs.append(legacy_shared)
...
def _x_wait_for_post_button(page, root, has_media: bool):
    ...
    for elapsed in range(timeout_s):
        ...
        try:
            media_ready = not has_media or not last_state or not last_state.get("processing")
            if post_btn and post_btn.is_enabled() and media_ready:
